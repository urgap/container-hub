"""Basic tests for compresstozip container."""
...
def test_compresstozip_nodes_registered():
    """Test that CompressToZip nodes are discovered by urgap."""
    result = subprocess.run(
        ["python", "-c",
         "import urgap; "
         "lookup = urgap.instances.unode_manager.wrapper_lookup; "
         "print(all(node in lookup for node in ["
         "'CompressToZip:1.0.0', 'CompressToZip:latest']))"],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0
